...
<body class="h-screen w-screen bg-black text-white flex flex-col justify-between overflow-hidden">
    <header class="h-20 bg-gray-900/90 border-b border-gray-800 px-8 flex justify-between items-center z-50">
        <div class="flex items-center space-x-4">
            ...
            <div>
                <h1 class="text-xl font-bold uppercase">{{ $schoolName }}</h1>
                <p class="text-xs text-blue-400">{{ $schoolTagline }}</p>
            </div>
        </div>
        <div id="clock" class="text-2xl font-bold text-blue-400"></div>
    </header>

    <main class="flex-1 relative w-full h-[calc(100vh-10rem)] flex items-center justify-center overflow-hidden">
        @forelse($activeSliders as $i => $s)
            <div class="slide absolute inset-0 w-full h-full flex items-center justify-center transition-opacity duration-700 {{ $i === 0 ? 'opacity-100 z-20' : 'opacity-0 z-10' }}" data-duration="{{ $s->duration_seconds }}">
                ...
                @else
                    <div class="p-8 bg-gray-900 rounded-2xl text-center"><h2 class="text-2xl">{{ $s->title }}</h2><audio src="{{ $s->media_url }}" controls autoplay class="mt-4"></audio></div>
                @endif
                <div class="absolute bottom-6 left-8 bg-black/80 px-4 py-2 rounded-xl text-sm font-bold">{{ $s->title }}</div>
            </div>
        @empty
            <div class="text-gray-500 text-xl">Mading Digital Aktif</div>
        @endforelse
    </main>

    <footer class="h-14 bg-blue-950 border-t border-blue-600 flex items-center overflow-hidden z-50">
        <span class="px-4 py-2 bg-blue-600 text-xs font-bold shrink-0">⚡ PENGUMUMAN</span>
        <div class="overflow-hidden w-full"><div class="marquee text-sm font-bold text-blue-100 space-x-12">
